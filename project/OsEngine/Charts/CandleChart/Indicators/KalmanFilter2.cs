...
{
    public class KalmanFilter2 : IIndicator
    {
        ...
        public KalmanFilter2(string uniqName, bool canDelete)
        {
            Name = uniqName;
            K = 2;
            Sharpness = 2;
        ...
        /// </summary>
        /// <param name="canDelete">whether user can remove indicator from chart manually/можно ли пользователю удалить индикатор с графика вручную</param>
        public KalmanFilter2(bool canDelete)
        {
        ...
        public decimal GetValue(List<Candle> candles, int index)
        {
            ...
            decimal shk = Convert.ToDecimal(Math.Sqrt(Convert.ToDouble(Sharpness * K / 100)));
            decimal distance = candles[index].Close - Values[index - 1];                      
            _velocity[index] = _velocity[index - 1] + distance * K / 100;
            decimal error = Values[index - 1] + distance * shk;
            return error + _velocity[index];
        }
        List<decimal> _velocity = new List<decimal>();
    }
}