            ...
            try
            {
                using (StreamReader reader = new StreamReader(@"Engine\" + Name + @".txt"))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (indicator[0] == "KalmanFilter2")
                        {
                            CreateIndicator(new Indicators.KalmanFilter2(indicator[1], Convert.ToBoolean(indicator[3])), indicator[2]);
                        }
                        if (indicator[0] == "KalmanFilter")
                        {
                            CreateIndicator(new Indicators.KalmanFilter(indicator[1], Convert.ToBoolean(indicator[3])), indicator[2]);
                        }
